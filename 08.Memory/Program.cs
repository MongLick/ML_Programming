namespace _08._Memory
{
	internal class Program
	{
        ...
        */
		...
		// 상수 같은거 게임 이름이나 최대 인원 수 등 절대 바뀌지 않는 것
        ...
        */
		...
		// a는 전부 다른 함수에 존재하기에 다 다른 a이다.
		// Main a 와 Stack1 a 와 aStack2 a는 전부 다 다른 a이다.
        ...
        */
		...
		void Main2()
		{
			...
			HeapClass local;              // 함수 시작시 이미 메모리에 할당되어 있음					// 이거는 변수이고 그래서 주소지를 가르킬 수 있는애다.
			local = new HeapClass();      // 인스턴스를 힙영역에 생성하고 주소값을 local에 보관			// new 한게 인스턴스이다. 그래서 new 가 힙영역에 저장된다. 인스턴스 == 객체

			// 함수 종료시
			// 지역변수 local 이 함수 종료와 함께 소멸됨
			// 인스턴스 new HeapClass() 는 함수 종료와 함께 더이상 참조하는 변수가 없음
			// 인스턴스 new HeapClass() 는 가비지가 되어 가비지 컬렉터가 동작할 때 소멸됨
			// 하나 하나 계속 삭제하는게 아니라 마크를 해놓고 한 번에 삭제한다.
			// 가비지컬렉터 = 지우는 애
			// 가비지컬렉션 = 지우는 행위
		}
        ...
        */
		...
		// 정적변수 : 클래스의 이름을 통해 접근하는 변수 == 어디서든 그냥 쓸 수 있다.
		// 근데 몬스터를 100마리를 만든다고 할 때 1마리만 나와도 되는 상황 같은 경우 생성하고 삭제하는게 더 효율적이기에 모든걸 정적으로만 하지는 않는다.
		// 정적함수 : 클래스의 이름을 통해 접근하는 함수
		// 정적클래스 : 정적변수와 정적함수만을 포함하는 클래스 == 정적 변수 함수가 아니면 사용할 수 없다.

		class Student
		{
			private static int count;
			// 원래 Student.count를 해야하지만 Student 클래스 안에 있기에 Student를 안 쓰고 단순화해서 쓸 수 있다. 다른 클래스에 써야한다면 Student. 으로 써야한다.
			private int id;
		...
		// 정적은 클래스 이름 점으로 사용하면 되고
		// 정적으로 안 만들었으면 new해서 인스턴스해서 사용하면 된다.
		// 스택은 고정적인 데이터 값이 있기에 고정된 값을 생성 삭제하면 되기에 힙영역보다 빠르다.
		// 힙영역은 어디 메모리가 비어있는지 찾고 거기에 new인 인스턴스를 넣어야 하기에 좀 느리다. 그리고 빈칸이 있으면 앞으로 떙기기 때문에 시간 소요가 좀 더 있다.
