...
namespace _06._UserDefineType
{
	internal class Program
	{
		...
		static void Main()
		{
			...
			float averagekim = kim.Average();
			float averagejong = jong.Average();

			Console.WriteLine(averagekim);
			Console.WriteLine(averagejong);
		}
		...
		struct Point
		{
			...
			public Point(int x, int y)		// 초기화 함수
			{
		...
		void Main4()
		{
			...
			Point point2 = new Point();                     // 기본 초기화 사용            // 이거나
			Console.WriteLine($"{point2.x}, {point2.y}");   // output : 0, 0

			Point point3 = new Point(1, 2);                 // 추가로 구현한 초기화 사용    // 이거를 보통 쓴다.
